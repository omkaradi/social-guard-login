...
export const SSOButtons = () => {
  ...
  const handleSSOLogin = async (providerId) => {
    ...
    try {
      ...
      toast({
        title: t('signInWith', { provider: ssoProviders.find(p => p.id === providerId)?.name }),
        description: "Demo: SSO login would redirect to provider authentication",
      });
    } catch (error) {
      toast({
        title: t('error'),
      ...
      <div className="grid grid-cols-3 gap-3">
        {ssoProviders.map((provider) => (
          <Button
            ...
            className={`h-12 transition-smooth shadow-button-sso hover:shadow-button-sso hover:scale-105 ${provider.color}`}
          >
            ...
            <span className="sr-only">{t('signInWith', { provider: provider.name })}</span>
          </Button>
        ))}
      </div>
    </div>
